...
class CalcsController extends AppController
{
    ...
    public function getResults()
    {
        ...
        $territories = $this->filterTerritoryData($domain_id,$study_id,$scenario_id,$study_case,$municipality);
    ...
    private function filterTerritoryData($domain_id, $study_id, $scenario_id, $study_case, $municipality)
    {
        if ($study_case > 1)
        {
            if($municipality == '')
            ...
            else
            {
                ...
                $territory_study_values = $this->TerritoriesDomains->find('all',['conditions' => ['domain_id = ' => $domain_id]])->select('id','territory_id')
                    ->contain(['Territories' => ['conditions' => ['Territories.parish is  not null','Territories.municipality = ' => $municipality],'fields' =>['id','name']]])
                    ->contain(['Scenarios' => ['conditions' => ['Scenarios.id = ' => $scenario_id],'fields' => ['id', 'ScenariosTerritoriesDomains.territory_domain_id']]])
                    ->contain(['Studies' => ['conditions' => ['Studies.id = ' => $study_id],'fields' => []]]);
            ...
            $territory_study_values = $this->TerritoriesDomains->find('all',['conditions' => ['domain_id = ' => $domain_id]])->select('id','territory_id')
                ->contain(['Territories' => ['fields' =>['id','name']]])
                ->contain(['Scenarios' => ['conditions' => ['Scenarios.id = ' => $scenario_id],'fields' => ['id','ScenariosTerritoriesDomains.territory_domain_id']]])
                ->contain(['Studies' => ['conditions' => ['Studies.id = ' => $study_id],'fields' => []]]);
