    ...
    <div class="row table-responsive">
        <table class="table vertical-table">
            <tr>
                ...
                <td><?= $scenario->has('domain') ? $this->Html->link($scenario->domain->name, ['controller' => 'Domains', 'action' => 'admin_view', $scenario->domain->id]) : '' ?></td>
            </tr>
            <tr>
    ...
    <div class="row">
        ...
        <?php if (!empty($scenario->territories_domains)): ?>
            <div class="table-responsive">
                ...
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['controller' => 'TerritoriesDomains', 'action' => 'admin_view', $territoriesDomains->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['controller' => 'TerritoriesDomains', 'action' => 'admin_edit', $territoriesDomains->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['controller' => 'TerritoriesDomains', 'action' => 'admin_delete', $territoriesDomains->id], ['confirm' => __('Are you sure you want to delete # {0}?', $territoriesDomains->id)]) ?>
                </td>-->
                        </tr>
                    <?php endforeach; ?>
                </table>
